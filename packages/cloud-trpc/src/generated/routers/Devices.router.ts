...
export const devicesRouter = t.router({
  ...
  deleteOneDevices: publicProcedure
    .input(DevicesDeleteOneSchema).mutation(async ({ ctx, input }) => {
      const deleteOneDevices = await ctx.prisma.devices.delete(input);
      return deleteOneDevices;
    }),
  findFirstDevices: publicProcedure
    .input(DevicesFindFirstSchema).query(async ({ ctx, input }) => {
  ...
  findManyDevices: publicProcedure
    .input(DevicesFindManySchema).query(async ({ ctx, input }) => {
      const findManyDevices = await ctx.prisma.devices.findMany(input);
      return findManyDevices;
    }),
  findUniqueDevices: publicProcedure
    .input(DevicesFindUniqueSchema).query(async ({ ctx, input }) => {
  ...
  groupByDevices: publicProcedure
    .input(DevicesGroupBySchema).query(async ({ ctx, input }) => {
      const groupByDevices = await ctx.prisma.devices.groupBy({ where: input.where, orderBy: input.orderBy, by: input.by, having: input.having, take: input.take, skip: input.skip });
      return groupByDevices;
    }),
  updateManyDevices: publicProcedure
    .input(DevicesUpdateManySchema).mutation(async ({ ctx, input }) => {
  ...
  updateOneDevices: publicProcedure
    .input(DevicesUpdateOneSchema).mutation(async ({ ctx, input }) => {
      const updateOneDevices = await ctx.prisma.devices.update(input);
      return updateOneDevices;
    }),
  upsertOneDevices: publicProcedure
    .input(DevicesUpsertSchema).mutation(async ({ ctx, input }) => {
